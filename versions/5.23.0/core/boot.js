// 这是一个启动顺序文件
// -----------------------
// - 这是一个大文件，这样就不必到处遵循启动逻辑
// - 该文件充满了调试语句，因此我们可以看到各个步骤的计时，因为启动需要尽可能快
// - 当我们设法将代码库分解为不同的组件时，例如 前端，他们的启动逻辑可以卸载给他们
// - app.js 是独立的，因为每个组件都有自己的 app.js 文件与其并置，而不是在服务器/web 内部
//
// IMPORTANT:
// ----------
// 这里唯一的全局要求是:覆盖+调试，这样我们就可以使用 DEBUG = Ghost: boot * Node Ghost 来监视计时
...
async function initCore({ghostServer, config, bootLogger, frontend}) {
    ...
    // URL Utils 有点慢，把它放在这里，以便时间与模型分开可见
